package de.h_da.fbi.nzse.thediaryappv2.entry.action;
...
import de.h_da.fbi.nzse.thediaryappv2.entry.NewNoteActivity;
...
public class NewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            RoomDatabaseAPI.databaseExecutor.execute(() -> incomingEntry = db.entryDao().getLastId());
        }
        catch (Exception e){
    ...
    private void addActionEntry() {
        if(!actionEntryCrossRefSortedMap.isEmpty() && actionEntryCrossRefSortedMap.size() <= MAX_ACTIONS) {
            actionEntryCrossRefSortedMap.forEach((actionId, crossRefId) -> {
                RoomDatabaseAPI.databaseExecutor.execute(() -> db.actionEntryDao().insert(crossRefId));
                startActivity(new Intent(NewActivity.this, NewNoteActivity.class).putExtra("Entry",incomingEntry));
            });
        }
        else{
